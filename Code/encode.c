...
static void show_letter_on_display(char c)
{
    ...
    for(int k = 0; pat[k] && j < 16; k++, j++)
        disp[j] = pat[k];    // stores the Morse pattern of the letter
    while(j < 16) disp[j++] = ' ';
    disp[16] = '\0';
...
static void lcd_add_char(char c)
{
    if(first_key)
    {
		...
		// once line1 is full (16 characters), scroll left
    if(lcd_col >= 16){
        // scroll left
...
void Encode_Run(void)
{
    ...
    if(confirmed != 0)
    {
        if(confirmed == '#') // word gap
        {
            Encode_Queue_Push('#');
        }
        else if(Morse_Encode(confirmed) != NULL) // if letter is real
        {
    ...
    if(g_enc_state == ENC_IDLE && !queue_empty())
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // clear line 2 then show "[letter]: [morse pattern]"
