...
{
    // PC13 pressed (blue button on NUCLEO, falling edge detected in ISR)
    ...
    // PC13 released (rising edge detected in ISR)
    if(g_flag_sw4_released)
    {
        g_flag_sw4_released = 0;
        LED0_OFF();

				// duration of press equates to dot or dash...
				// < 300ms = dot, >= 300ms = dash
    ...
    if(g_flag_tim6_fired)
    {
        ...
        else
        {
            ...
            if(found == '?')   // no character per the Morse pattern given
            {
            ...
            // clear buffer and line 2 for next letter
						// whenever new symbols pressed, past ones are cleared
            g_dec_buf_idx = 0;
            g_dec_buf[0]  = '\0';
            LCD_Set_Cursor(1, 0);
            LCD_Write_String("                ");
