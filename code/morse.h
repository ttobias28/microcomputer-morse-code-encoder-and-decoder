 ...
 *   PD3  = SW5 on EDUBase     = mode toggle (encode <-> decode)
 */
...
/* =========================================================
 *  system clock
 *  MSI oscillator at 4 MHz after HAL_Init() + System_Clock_Config().
 * ========================================================= */
 ...
 *  morse timing
 *
 *  international Morse standard (1 unit = 200 ms via TIM2):
 *    dot            = 1 unit
 *    dash           = 3 units
 *    intra-symbol   = 1 unit  (gap between dots/dashes same letter)
 *    inter-letter   = 3 units
 *    inter-word     = 7 units
 * ========================================================= */
#define DOT_TICKS        1
#define DASH_TICKS       3
...
/* TIM2: 200 ms period
 *   period = (PSC+1)*(ARR+1) / 4MHz = 4000*200/4000000 = 0.2 s
 */
#define TIM2_PSC  3999
#define TIM2_ARR   199

/* TIM3: free-running, 1 count = 1 ms */
...
/* TIM6: 600 ms one-shot decode timeout
 *   period = (3999+1)*(599+1)/4000000 = 0.6 s
 */
 ...
 *  if the user stops tapping for this many ms, confirm letter.
 *  implemented as a loop-count in keypad.c.
 * ========================================================= */
...
/* =========================================================
 *  shift register pins (shared between 7-seg and LCD)
 * ========================================================= */
...
/* =========================================================
 *  keypad pins
 *  columns PB1-PB4 = outputs
 *  rows    PB8-PB11 = inputs
 * ========================================================= */
 ...
 *        use pull-up, detect FALLING edge = press, RISING = release
 *        EXTI line 13 -> EXTI15_10_IRQHandler
 *
 *  SW5 = PD3 = EDUBase push button
 *        active HIGH (pressed = 3.3V)
 *        use pull-down, detect RISING edge = press
