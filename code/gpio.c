...
void GPIO_Init_All(void)
{
     ...
     * note: GPIOD not needed anymore since mode toggle moved to PB8 */
    ...
    SET_OUTPUT(GPIOA, 1);  /* LED0 PA1 */
    SET_OUTPUT(GPIOA, 0);  /* LED1 PA0 */
    SET_OUTPUT(GPIOC, 7);  /* LED2 PC7 */
    SET_OUTPUT(GPIOC, 8);  /* LED3 PC8 */
    ...
    /* PB8 = mode toggle button (same as lab 9/10)
     * active HIGH - needs pull-down
     * note: keypad rows also use PB8-PB11 as inputs with no pull.
     * PB8 pull-down is set here; Keypad_Init() will clear it to no-pull.
     * EXTI_SW5_Init() sets up the interrupt on top of this. */
    SET_INPUT_PULLDOWN(GPIOB, 8);

    /* keypad pins initialised in Keypad_Init() */
}